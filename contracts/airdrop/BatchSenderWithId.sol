 ...
 * It now includes a unique, auto-incrementing ID for each transfer.
 */
contract BatchSender is Ownable {
  address public admin;
  uint256 public transferCounter = 1;
  ...
  function batchSend(
    ...
    for (uint256 i = 0; i < recipientsCount; i++) {
      ...
      emit TransferSent(_token, _recipients[i], _amounts[i], transferCounter);
  ...
  /**
   * @dev Allows the admin to withdraw a specific amount of ERC20 tokens.
   * @param _token The address of the ERC20 token to withdraw.
